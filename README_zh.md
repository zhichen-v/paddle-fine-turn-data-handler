...
這個 app 不會執行 OCR。它只負責把最耗時的人工標註流程做順：匯入 PDF/圖片、轉成可標註影像、手動畫框、手動輸入文字、儲存 project，最後匯出 PaddleOCR detection 訓練格式。
...
- Project 儲存：本地 JSON，放在 `data/projects`
- Dataset 匯出：本地檔案，放在 `data/exports`
...
`data/projects`、`data/exports`、`data/uploads` 都已加入 `.gitignore`。
